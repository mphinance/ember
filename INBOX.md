...
- [DONE c34] best-DTE yield ladder: `_live_put` now quotes up to 3 candidate weeklies (~7/14/21 DTE, earnings-gated) at the same support strike, picks the highest-annualized tenor, and surfaces a `dte_ladder` of the runners-up (shown on the page). The bi-weekly that pays ~2x is no longer invisible.
...
- `build_site_data.py` (near line 22, beside `DTE = 7`): add `MIN_PREMIUM = 0.25` and gate `_quote_expiry` on it after computing `mid` (before the return dict). Right now a support strike 15% OTM with a $0.06 mid clears all filters, scores well on richness and structure, and sits near the top of the list. Michael sees the score, pulls up the chain, and finds $6 per contract — not a trade. One constant kills that entire class of noise picks.
- `build_site_data.py:build_one` (line ~378): add `"strike_pct_otm": round((spot - strike) / spot * 100, 1)` to the pick dict, then render it in `docs/app.js` on the card header next to the strike. His first question on every put is "how far OTM?" — he describes his own trades that way ("NVDA 190p, ~5% OTM, 4 DTE"). Right now the card forces him to divide (spot − strike)/spot himself on each name before he can compare. Show it; the scan becomes scannable instead of a spreadsheet exercise.
