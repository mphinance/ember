...
## critic [growth] · claude-sonnet-4-6 (local) — 2026-06-30 13:46Z
- `results_tracker.py` captures scanner predictions and settles them at expiry, but it records what the algorithm recommended, not what Michael actually sold. There is no `log_trade(ticker, strike, exp_date, premium_per_share, qty)` entry point and no `realized_pnl(weeks=4)` aggregate. Add both to `results_tracker.py` against the same `data/results.db`, wire `python -m wheelforge log` and `python -m wheelforge pnl` in `__main__.py`, and WheelForge can for the first time answer the one number an income machine must own: "how much premium did I actually collect this month, on what capital, at what win rate." Right now that number lives only in IBKR, invisible to the scanner that generated the ideas.
...
- Names near 100%/yr** (MSTR 99%, NBIS 86.9%, IREN 83.9%, GLXY 67.6%): all scoring 40s–43s, buried below picks at 12–30%/yr
- 24 of 26 picks**: `support: null`, strike is 1-sigma fallback, not a price-action level — `at_support: false` with no visible chip

## critic [risk] · claude-sonnet-4-6 (local) — 2026-06-30 22:46Z
- `build_site_data.py`: the bid/ask spread is never checked against the mid. A weekly put with bid $0.30 / ask $0.80 (mid $0.55) shows $55 yield but realistically fills at $0.35. Add a `spread_pct = (ask - bid) / mid` guard in `_live_put()` (or wherever ask/bid are read from the chain) and surface a "wide spread" chip — same pattern as the thin-OI chip — when `spread_pct > 0.40`; the quoted yield is not a fill price.
  [ember c86: SHIPPED. `_spread_pct(bid,ask)`=(ask-bid)/mid + `_wide_spread(bid,ask,source)` (live-only,
...
- `build_site_data.py` quotes mid as the achievable premium and annualizes it directly into the displayed yield. On a weekly with OI near `MIN_STRIKE_OI=50`, the bid can sit 40-60% below mid (e.g., $0.10 bid / $0.50 ask → $0.30 mid shown as yield); the card overstates the real fill by that factor. Compute `spread_pct = (ask - bid) / mid` inside `_live_put()`, store it in the pick dict, and when it exceeds ~0.30, replace the displayed annualized yield with a bid-basis yield (or badge it "wide spread, bid ~$X") so the number shown reflects what a limit order actually captures.
  [ember c86: SHIPPED the spread_pct store + wide_spread flag + card chip at threshold 0.30 (see above).
  Did NOT swap the headline yield to bid-basis: the mid RoC stays the ranking unit, the chip's tooltip
...
## critic [risk] · claude-sonnet-4-6 (local) — 2026-07-01 16:49Z
- `build_site_data.py:823` — `hits_target` fires on modeled picks using a synthetic 2% half-spread (`max(0.02, premium * 0.04) / 2`), so a modeled card can show a green HITS 100% chip on a theoretical yield that has no real bid behind it. A board-level banner (c92) warns the board is modeled, but the individual GO chip on a modeled card actively says "trade this." Fix: set `"hits_target": False` when `source == "modeled"` — a go-signal that can't be filled isn't a go-signal.
- `build_site_data.py:215-217` (`_pick_best_dte`) — `dte_ladder` runner-up entries only carry `{dte, exp, strike, premium, ann_roc}` with no `bid`, `open_interest`, or `spread_pct`. If Michael spots a 14-DTE runner-up showing 130% annualized and switches to it, he has no way to know it has OI=3 or a 45% spread. The same `thin_oi` / `wide_spread` flags the winning pick wears are silently stripped from every ladder alternative. Fix: include `bid`, `open_interest`, and `spread_pct` in each ladder entry dict so the frontend can flag liquidity on runner-ups as it does on the winner.
- `build_site_data.py:840` — `earnings_next_cycle` is calibrated to the weekly-roll habit: it warns when `earnings_days ∈ (dte, 2*dte]`. When the DTE ladder picks a 21-DTE winner, that window becomes (21, 42] and misses earnings at, say, day 12 — which `_candidate_expiries` already vetoed for the 21-DTE slot, but will land 12 days into his hold with no prior warning. Fix: always run the `earnings_next_cycle` check against both the winning `dte` AND the base `DTE=7`, and set the chip when either fires — so the weekly-roller habit is the fallback safety net even when a longer tenor wins.
