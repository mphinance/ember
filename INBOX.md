...
- `freeshares.py:65` — `_ramp(annualized_roc, 0.08, 0.35)` in `wheel_fit` is a dead constant. Michael's weekly book runs 100-200% annualized, so every live pick saturates this sub-factor to 1.0 and it never discriminates between a fat weekly and a thin monthly. `scoring.py` already fixed its ceiling to 2.0 in a prior cycle; `wheel_fit` was never updated. Change 0.35 → 2.0 here.
